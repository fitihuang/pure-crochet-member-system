...
- `SPEC.md` 是 repo 裡跟著 git 版控的檔案，專案的 `CLAUDE.md`「相關文件」段落會指向它，兩份文件分工：`CLAUDE.md` 放操作慣例跟踩過的坑，細節一律以 `SPEC.md` 為準。
- `SPEC.md` 同時也發布成一份 Claude Artifact，方便不碰程式碼的人（例如負責人）直接看網頁版，網址記錄在 `CLAUDE.md` 裡。
- **這份文件內容更新後，要記得同步重新發布到同一個 Artifact 網址**（用同一個 `url` 更新，不要另外開新的），避免兩邊內容兜不起來。
